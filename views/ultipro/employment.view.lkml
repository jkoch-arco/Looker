view: employment {
  ...
  sql_table_name: (SELECT Row_number() OVER(partition BY employeeid ORDER BY EmploymentStatus ASC,  statusstartdate DESC, terminationdate ASC) as most_recent_employee_record, Employment.* FROM ultipro.Employment WHERE LOAD_TS = (Select MAX(LOAD_TS) FROM ultipro.Employment)) ;;
